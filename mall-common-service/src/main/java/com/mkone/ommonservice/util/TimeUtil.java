...
import com.mkone.ommonservice.constant.SeckillTimeIntervalEnum;
...
public class TimeUtil {
     ...
     */
    private final static String[] formatStr = {FMT_yyyyMMddHHmmssS, FMT_yyyyMM, FMT_yyyyMMddHHmmss, FMT_yyyyMMddHHmm,
    ...
    public static Date getSystemDate(String fmtstr) {
        try {
            return parseToDate(formatDateTimetoString(getSystemDate(), fmtstr), fmtstr);
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @return
     */
    public static long getMonthNoAndWeekNo(LocalDateTime sourceTime) {
    ...
    /**
     * 返回该时间所对应的时间字符串，格式yyyyMMddHHmmss
     *
     * @param d 日期对象
    ...
    public static String getYyyymmddhhmmss(Date d) {
        ...
        String a = getYearOfDate(d) + "";//年
        String b = getMonthOfYear(d) + "";//月
        String c = getDayOfMonth(d) + "";//日
        String e = getHoursOfDay(d) + "";//时
        String f = getMinutesOfHour(d) + "";//分
        String g = getSecondsOfMinute(d) + "";//秒
    ...
    public static int getMonthCount(Date sDate, Date eDate) {
        String sDateStr = TimeUtil.formatDateTimetoString(sDate, "MM");
        String eDateStr = TimeUtil.formatDateTimetoString(eDate, "MM");
    ...
    public static int getYearCount(Date sDate, Date eDate) {
        String sDateStr = TimeUtil.formatDateTimetoString(sDate, "yyyy");
        String eDateStr = TimeUtil.formatDateTimetoString(eDate, "yyyy");
    ...
    public static Date getDayNextMonth(Date date) {
        String yearStr = TimeUtil.formatDateTimetoString(date, "yyyy");
        String monthStr = TimeUtil.formatDateTimetoString(date, "MM");
        String dayStr = TimeUtil.formatDateTimetoString(date, "dd");
        ...
        try {
            date = TimeUtil.parseToDate(dateStr);
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * 获取月份开始时0点0分0秒
     *
     * @param dataStr 日期字符串，例：2017-01
     * @return 日期对象
     */
    public static Date getMouthStart(String dataStr) {
        Date d = null;
        try {
            d = parseToDate(dataStr, "yyyy-MM");
        } catch (Exception e) {
            e.printStackTrace();
        }
        d = setDayOfDate(d, 1);
     ...
     * @return 日期对象
     */
    public static Date getMouthEnd(String dataStr) {
        Date mouthStart = getMouthStart(dataStr);
        Date nextMouthStart = addMonths(mouthStart, 1);
        return addDays(nextMouthStart, -1);
    }
     ...
     * @return 日期对象
     */
    public static Date getCurrentMouthStart(Date date) {
        date = setDayOfDate(date, 1);
        date = setHourOfDate(date, 0);
        date = setMinuteOfDate(date, 0);
        date = setSecondOfDate(date, 0);
        return date;
    }
    ...
    public static Date getCurrentMouthEnd() {
        Date currentMouthStart = getCurrentMouthStart();
        Date nextMouthStart = addMonths(currentMouthStart, 1);
        return addDays(nextMouthStart, -1);
    }
     ...
     * @return 日期对象
     */
    public static Date getCurrentMouthEnd(Date date) {
        Date currentMouthStart = getCurrentMouthStart(date);
        Date nextMouthStart = addMonths(currentMouthStart, 1);
        return addDays(nextMouthStart, -1);
    }
    ...
    public static int compareDate(Date startDay, Date endDay, int stype) {
        int n = 0;
        endDay = endDay == null ? getSystemDate("yyyy-MM-dd") : endDay;
    ...
    //计算当前时间所在的时间段，计算下一时间段
    public static Map<String, Date> getNextTime() {
        int start = 0;
        int end = 0;
        ...
        LocalDateTime startTime = now.of(now.getYear(), now.getMonth(), now.getDayOfMonth(), start, 0, 0);
        LocalDateTime endTime = now.of(now.getYear(), now.getMonth(), now.getDayOfMonth(), end, 0, 0);
        Date startData = asDate(startTime);
        map.put("startData", startData);
        Date endData = asDate(endTime);
        map.put("endData", endData);
        return map;
    }

    //LocalDateTime 转换 Data
    ...
    public static Map<String,Long> getNowTime(){
        int start = 0;
        int end = 0;
        ...
        LocalDateTime endTime = now.of(now.getYear(), now.getMonth(), now.getDayOfMonth(), end, 0, 0);
        Date startData = asDate(startTime);
        map.put("startData", startData.getTime());
        Date endData = asDate(endTime);
        map.put("endData", endData.getTime());
        return map;
